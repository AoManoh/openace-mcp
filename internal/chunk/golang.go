...
// splitOversized 把超出字节预算的声明按行细分为多个 AST capability chunk；
// 每段行数按预算与平均行宽估算，保证输出有界。
func (p Profile) splitOversized(relPath string, language string, capability Capability, startLine int, endLine int, text string, symbol string) []Chunk {
	if len(text) <= p.MaxChunkBytes || startLine == endLine {
	...
	for offset := 0; offset < totalLines; offset += step {
		...
		chunks = append(chunks, p.build(relPath, language, capability, startLine+offset, startLine+last-1, part, symbol))
	}
	return chunks
}
