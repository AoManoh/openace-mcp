...
- 推荐默认使用单租户配置；这是当前最稳定、最清晰的日常使用路径。
- 实验性支持可选 `provider_profile_id` 显式选择 ACE provider profile；同一 daemon 只共享控制面，provider 的 ACE client、checkpoint state、inflight、watch 和上游退避状态互相隔离。
...
如果你只有一个 Augment / ACE 账号或租户，不要配置 `OPENACE_PROFILES_FILE`。保持单租户配置能减少状态维度，也最接近当前稳定路径。

## 默认推荐：单租户配置

多数用户只需要配置一个租户。推荐使用 `OPENACE_MODE=auto`，让 AI IDE 启动 `openace-mcp` 后自动复用或托管本机 daemon：
...
也可以只设置 `OPENACE_SESSION_FILE`，或者依赖已登录插件留下的 `~/.augment/session.json`。单租户模式下不需要理解 `provider_profile_id`，也不需要给 MCP 工具传这个参数。

## 实验性能力：多 Provider Profile

多 provider profile 是高级/实验性能力，用于显式测试、排障或手动切换多个 ACE 账号。它不是自动高可用系统。

默认情况下 openACE 沿用单账号凭据链。只有在确实需要同一个 daemon 识别多个账号时，才配置本地 profile 文件：
...
- openACE 不会因为某个 profile 出现 `429`、quota exhausted、封禁或临时不可用而自动切换到另一个 profile。
- Augment / ACE 的代码索引跟随账号、租户和 checkpoint；另一个账号有 token，不代表它对同一 workspace 已经 ready。
- 当前实现只提供显式选择、状态隔离和状态可见性，是否切换由用户或调用方决定。
- 如果未来要做真正高可用，必须先保证备用租户已经对同一 workspace 独立预热，并且状态可证明为 ready。

在 MCP 调用中显式选择备用 profile 的示例：
...
如果备用 profile 还没有同步过这个 workspace，先显式预热：
...
对应工具是 `sync_workspace` 或 `start_sync_workspace`。预热完成后，用 `workspace_status` 观察该 profile 的 `provider_state`、`checkpoint_id` 和 `upstream_status`，不要把 cold 或 backoff 的 profile 当成可用替代。
...
稳定使用和排障复现时不要使用浮动 `@main`；请替换为发布 tag 或已验收 commit，例如 `@v0.1.0` 或 `@<commit>`。只有明确想跟随开发分支时才使用 `@main`。

```json
"openace-mcp": {
  "args": [
    "run",
    "github.com/AoManoh/openace-mcp/cmd/openace-mcp@<release-tag-or-commit>"
  ],
  "command": "go",
  "disabled": false,
  "env": {
...
```bash
GOPROXY=https://goproxy.cn,direct go install github.com/AoManoh/openace-mcp/cmd/openace-mcp@<release-tag-or-commit>
```
...
- 多 provider 仍是实验性/高级场景；压测或排障时可以显式传 `provider_profile_id`，但不要把它当作自动 failover 开关。
...
| `OPENACE_PROFILES_FILE` | 实验性多 provider profile JSON；设置后替代单账号凭据链，工具通过 `provider_profile_id` 显式选择。单租户用户应保持为空 |
